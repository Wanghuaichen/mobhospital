  ...
  $.fn.PieMenu = function(options) {
	...
	var clickHandler = function() {
		if($(this).parent().hasClass('active')){
			$('.menu_button').css('background','#fff url(/tiantan_emr/Public/css/images/jiaodian/tianjia.png) no-repeat center center')
			setPosition(0);
			$(this).parent().removeClass('active');
			$(this).parent().addClass('inactive');

		}else{
			$('.menu_button').css('background','url(/tiantan_emr/Public/css/images/jiaodian/guanbi.png) no-repeat center center')
			setPosition(1);
	...
	return settings.menu_element.each(function(i,ele){
		ele_angle[i] = (parseInt(settings.starting_angel) + angle*(i))*Math.PI/180;
		 x_pos[i] = (settings.radius * Math.sin(ele_angle[i])*1.25);
         y_pos[i] = (settings.radius * Math.cos(ele_angle[i])*1.25);
