...
;(function(){
	...
	$.extend({
		...
		"creatUrlPrint" : function(options){
			...
			//如果存在分页信息进行分页","以逗号作为分页符：
			url_arr = options.print_url.split(",");
			processed_url_arr = new Array();
			processed_url_arr[0] = url_arr[0];
			for(url_arr_count = 1;url_arr_count<url_arr.length;url_arr_count++)
			{
				processed_url_arr[url_arr_count] = url_arr[0]+","+url_arr[url_arr_count];
			}
			for(lodop_id_count=0;lodop_id_count<url_arr.length;lodop_id_count++)
			{
				LODOP[lodop_id_count]=getLodop(document.getElementById(options.object_id),document.getElementById(options.embed_id));
				if(options.is_new_LODOP)
				{
					LODOP[lodop_id_count].PRINT_INIT(options.print_setting_name);
				}
				if(options.print_url != "")
				{
					LODOP[lodop_id_count].ADD_PRINT_URL(options.top,options.left,options.width,options.height,processed_url_arr[lodop_id_count]);
					LODOP[lodop_id_count].SET_PRINT_MODE("PRINT_PAGE_PERCENT",options.print_page_percent);
				}
				else
				{
					alert("打印页面URL错误!");
...
function printerPrint(){
	...
	for(lodop_id_count=0;lodop_id_count<LODOP.length;lodop_id_count++)
	{
		if($(LODOP[lodop_id_count]).size() > 0)
		{
			if(part_print_start_page!==0&&part_print_mode==true)
			{
				LODOP[lodop_id_count].SET_PRINT_MODE ("PRINT_START_PAGE",part_print_start_page);
			}
			/*
			暂时取消对打印页数的控制
				
				if(end_page!=0)
					LODOP[lodop_id_count].SET_PRINT_MODE ("PRINT_END_PAGE",end_page);
			*/
			if(document_type.toLowerCase().indexOf("peiyaoka")!=-1||document_type.toLowerCase().indexOf("shuyeka")!=-1||document_type.toLowerCase().indexOf("yizhuzhixing")!=-1)
				LODOP[lodop_id_count].SET_PRINT_MODE("PRINT_PAGE_PERCENT","65%");
			if(lodop_id_count==0)
				LODOP[lodop_id_count].PRINTA();
			else
				{
					LODOP[lodop_id_count].SET_PRINT_MODE("CUSTOM_TASK_NAME","分页打印"+lodop_id_count);
					LODOP[lodop_id_count].PRINT();
				}
...
function printerPreview(){
	...
	if($(LODOP[lodop_id]).size() > 0)
	{
		...
		*/
		...
		//LODOP[lodop_id].SET_SHOW_MODE ("HIDE_PBUTTIN_PREVIEW",1);
