...
//添加新药品：
						...
						$("#add_tongzu").click(function(){
							...
							window.setTimeout(function(){
								...
								else
								{
									...
									$.post("http://"+server_url+"/tiantan_emr/"+group_name+"/Chufangguanli/addOneYaopin", { "chufang_id": last_chufang_id,"dachufanghao":dachufanghao,"type":"组合","zuhao":temp_zuhao},function(data){
										submit_id = data.id;
									}, "json");
									$.ajaxSetup({
										async: true
									...
									$('input:[name="zuhao[]"]:last').parent().parent().parent().after(
										'<table border="0" cellpadding="0" cellspacing="0" class="content_table" style="width:100%;margin-top:20px;">'+
											'<tr class="blank_yaopin_data" id="'+submit_id+'" zuhao="'+temp_zuhao+'">'+
												...
												'</td>'+
												'<td width="8%"><input style="width:50%;" type="text" name="danjia[]" value="" />元</td>'+
												'<td width="8%"><input style="width:50%;" type="text" name="xiaoji[]" value="" disabled="disabled"/>元</td>'+
				...
				art.dialog({
					...
					init: function () 
					{
						...
						$("#add_tongzu").click(function(){
							...
							$.post("http://"+server_url+"/tiantan_emr/"+group_name+"/Chufangguanli/addOneYaopin", { "chufang_id": last_chufang_id,"dachufanghao":dachufanghao,"type":"组合","zuhao":last_zuhao},function(data){
								submit_id = data.id;
							}, "json");
							$.ajaxSetup({
								async: true
							...
							$('input:[name="zuhao[]"]:last').parent().after(
								'<tr class="blank_yaopin_data" id="'+submit_id+'" zuhao="'+last_zuhao+'">'+
									...
									'<td width="8%"><input style="width:50%;" type="text" name="danjia[]" value="" />元</td>'+
									'<td width="8%"><input style="width:50%;" type="text" name="xiaoji[]" value="" disabled="disabled"/>元</td>'+
									'<td width="10%"><input style="width:80%;" type="text" name="fufeifangshi[]" value="" /></td>'+
									'<td width="10%"><input style="width:80%;" type="text" name="zhixingkeshi[]" value="" /></td>'+
									'<td width="10%" style="display: none;"><input style="width:80%;" type="text" name="shifou_zibei[]" value="" disabled="disabled"/></td>'+
									'<td width="10%">'+
										'<div style="width:80px;">'+
											'<input type="button" name="add_yaopin" class="add_chufang" value="">&nbsp;'+
	...
	$("[name='delete_yaopin']").live("click",function(){
		if($(".blank_yaopin_data").length!=0)
		{
			...
			$("#yaopin_form").find("[name='add_yaopin']:last").removeAttr("disabled");
...
//					}
					if(chufang_state=="新添加"&&$(this).find("[name='isfirst']").val()=='true')
					{
						...
						art.dialog({
							...
							title:"编辑处方",
							content:'<form style="width:750px;" class="ajax_form" method="post" action="http://'+server_url+'/tiantan_emr/'+group_name+'/Chufangguanli/updateOneYaopin">'+
													'<li>'+
					...
					else if(chufang_state=="新添加"&&$(this).find("[name='isfirst']").val()!='true')
					{
						art.dialog({
							id:"edit_chufang_dialog",
							title:"编辑处方",
							content:'<form class="ajax_form" method="post" action="http://'+server_url+'/tiantan_emr/'+group_name+'/Chufangguanli/updateOneYaopin">'+
													'<li>'+
													'<span >药品名称:</span><input type="text" name="yaopin_mingcheng" class="input_type_full" value="'
