...
/**************************************************
 * Created: 2015-12 Info:检验检查多媒体记录List Adapter
 * 
...
public class MultiMediaListAdapter extends BaseAdapter {
	...
	public MultiMediaListAdapter(Context context, List<Map<String, Object>> data) {
		this.listData = data;
	...
	public void addlistData(List<Map<String, Object>> listData2, boolean reflesh, int location) {
		...
		for (Map<String, Object> map : listData2) {
			if (!isExist(map)) {
				listData.add(map);
				if (map.get("report_type").equals("photo")) {
	...
	public View getView(int position, View view, ViewGroup parent) {
		...
		if (view == null) {
			...
			viewHolder.jiancha_report_img = jiancha_report_img;
			viewHolder.jiancha_report_time = jiancha_report_time;
			viewHolder.jiancha_report_vedio_audio = jiancha_report_vedio_audio;
			viewHolder.jiancha_report_vedio = jiancha_report_vedio;
		...
		if ((listData.get(position).get("report_path") != null)
				&& (listData.get(position).get("report_type").equals("photo"))) {

			// Bitmap bitmap = (Bitmap)
			// listData.get(position).get("report_img");
			LayoutParams para;
			para = viewHolder.jiancha_report_img.getLayoutParams();
			WindowManager wm = (WindowManager) context.getSystemService(Context.WINDOW_SERVICE);
			int screenwidth = wm.getDefaultDisplay().getWidth();
			// para.height = (IMG_WIDTH) * bitmap.getHeight() /
			// bitmap.getWidth();
			para.height = IMG_HEIGHT;
			// para.height = LayoutParams.WRAP_CONTENT;
			para.width = IMG_WIDTH;
			viewHolder.jiancha_report_img.setLayoutParams(para);
//			jiancha_report_img.setImageBitmap(bitmap);
			viewHolder.jiancha_report_vedio_audio.setVisibility(View.GONE);
			// jiancha_report_vedio_audio.setText(timeString + ".jpg");
			String imageURI = "file://" + listData.get(position).get("report_path");
			((YiDongYiHuMultiMediaActivity) context).imageLoader.displayImage(imageURI, viewHolder.jiancha_report_img,
					((YiDongYiHuMultiMediaActivity) context).options);

			viewHolder.jiancha_report_vedio.setVisibility(View.GONE);
			viewHolder.jiancha_report_time.setText("采集时间：" + timeString + "（图片）");
		} else {
			// jiancha_report_img.setVisibility(View.GONE);

			if (listData.get(position).get("report_type").equals("vedio")) {
				// MediaController mc = new MediaController(context);
				// MediaController mc = (MediaController)
				// view.findViewById(R.id.jiancha_report_vedio_crl);
				// mc.setAnchorView(jiancha_report_vedio);
				// mc.setMediaPlayer(jiancha_report_vedio);
				// mc.show();
				// jiancha_report_vedio.setMediaController(mc);
				String uid = GlobalInfoApplication.getInstance().appConf.current_patient_zhuyuan_id;
				String report_time = (String) listData.get(position).get("report_time");
				String vedioPath = MediaUtil.VEDIO_PATH_PREFIX + uid + File.separator + report_time + ".3gp";

				Bitmap bitmap = MediaUtil.loadCachePhoto(uid, report_time);
				if (bitmap == null) {
					bitmap = ThumbnailUtils.createVideoThumbnail(vedioPath, Video.Thumbnails.MINI_KIND);
					MediaUtil.saveCachePhoto(bitmap, uid, report_time);
				}
				...
				try {
					...
					WindowManager wm = (WindowManager) context.getSystemService(Context.WINDOW_SERVICE);
					int screenwidth = wm.getDefaultDisplay().getWidth();
					// para.height = (screenwidth - 10) * bitmap.getHeight() /
					// bitmap.getWidth();
					para.height = IMG_HEIGHT;
					para.width = IMG_WIDTH;
					viewHolder.jiancha_report_img.setLayoutParams(para);
					// jiancha_report_img.setImageBitmap(bitmap);
					viewHolder.jiancha_report_img.setImageDrawable(la);
					viewHolder.jiancha_report_img.setVisibility(View.VISIBLE);
				...
				viewHolder.jiancha_report_time.setText("采集时间：" + timeString + "（录像）");
			} else if (listData.get(position).get("report_type").equals("audio")) {
				// jiancha_report_vedio_audio.setText(timeString + ".amr");
				viewHolder.jiancha_report_vedio_audio.setVisibility(View.GONE);
				viewHolder.jiancha_report_vedio.setVisibility(View.GONE);
				viewHolder.jiancha_report_img.setImageResource(R.drawable.luyin);
				LayoutParams params = viewHolder.jiancha_report_img.getLayoutParams();
				params.width = IMG_WIDTH;
				params.height = LayoutParams.WRAP_CONTENT;
				viewHolder.jiancha_report_img.setLayoutParams(params);
				// jiancha_report_img.setBackgroundColor(Color.WHITE);

				viewHolder.jiancha_report_time.setText("采集时间：" + timeString + "（录音）");
			} else {
				viewHolder.jiancha_report_vedio_audio.setText(timeString + "");
				viewHolder.jiancha_report_vedio.setVisibility(View.GONE);
				viewHolder.jiancha_report_time.setText("采集时间：" + timeString + "（其他）");
			}
		}
