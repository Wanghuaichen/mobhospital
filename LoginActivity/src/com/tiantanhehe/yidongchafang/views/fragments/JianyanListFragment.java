...
public class JianyanListFragment extends Fragment {
	private View view;
	private String server_url, current_patient_zhuyuan_id,
			...
			jiancha_code, baogao_name, jiancha_keshi_name,
			current_patient_bingchuang_hao;
	private Button bt_shaixuan;
	private EditText showdate, edt_jianchamingcheng;
	private TextView detail;
	private Spinner spinner_jianchakeshi, pages, spinner_jiancha_zhuangtai;
	private ListView listView;
	private String url;
	private LinearLayout bottom_page;
	private Map<String, String> map;
	private List<Map<String, Object>> listData;
	private List<Map<String, Object>> jianyanlistData;
	private JianyanJianchaListAdapter dataAdapter;
	private boolean clickState = false;
	private String initEndDateTime = "2014-08-23"; // 初始化结束时间

	public static JianyanListFragment newInstance() {
		JianyanListFragment fragment1 = new JianyanListFragment();
		return fragment1;
	}
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		view = inflater.inflate(R.layout.jianyan_list, container,
				false);
	...
	private void viewInit() {
		...
		showdate = (EditText) view.findViewById(R.id.edt_shenqingshijian);
		 ...
		 * .format(new java.util.Date()); showdate.setText(initEndDateTime);
		 */
		...
		spinner_jiancha_zhuangtai = (Spinner) view.findViewById(R.id.spinner_jiancha_zhuangtai);
		String spinner_itme[] = getResources().getStringArray(
				R.array.jianyan_spinner);
		String spinner_zhuangtai[] = getResources().getStringArray(R.array.jianchazhuangtai_spinner);
		ArrayAdapter<String> zhuangtai_spinnerAdapter = new ArrayAdapter<String>(getActivity(),
				android.R.layout.simple_list_item_1, spinner_zhuangtai);
		ArrayAdapter<String> spinnerAdapter = new ArrayAdapter<String>(
				getActivity(), android.R.layout.simple_list_item_1,
				spinner_itme);
		spinner_jiancha_zhuangtai.setAdapter(zhuangtai_spinnerAdapter);
		spinner_jiancha_zhuangtai.setSelection(0, true);
		...
		bt_shaixuan.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				String jianchaZhuangtaiText = spinner_jiancha_zhuangtai.getSelectedItem().toString();
				if (!jianchaZhuangtaiText.equals("")) {
					map.put("jiancha_state", jianchaZhuangtaiText);
				} else {
					map.remove("jiancha_state");
				}
	...
	private void getData(String url, final Map<String, String> map) {
		...
		new HttpHelper(getActivity(), new IHandleHttpHelperResult() {
			...
			public void handleResult(final List<Map<String, Object>> httpdata) {
				listData = httpdata;
				jianyanlistData = new ArrayList<Map<String, Object>>();
				for (int i = 0; i < listData.size(); i++) {
					jianyanlistData.add(listData.get(i));
					dataAdapter = new JianyanJianchaListAdapter(
							jianyanlistData, getActivity());
					listView.setAdapter(dataAdapter);
					listView.setOnItemClickListener(new OnItemClickListener() {
						@Override
						public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
							// TODO Auto-generated method stub
							shenqing_time = listData.get(arg2).get("jiancha_time").toString();
							songjian_keshi = listData.get(arg2).get("shenqing_keshi_name").toString();
							songjian_yisheng = listData.get(arg2).get("shenqing_zhe_name").toString();
							songjian_biaoben = listData.get(arg2).get("songjian_wu").toString();
							jiancha_time = listData.get(arg2).get("jiancha_time").toString();
							jianyanz_zhe = listData.get(arg2).get("jiancha_zhe_name").toString();
							hedui_zhe = listData.get(arg2).get("hedui_zhe_name").toString();
							// leixing =
							// listData.get(arg2).get("leixing").toString();
							baogao_id = listData.get(arg2).get("id").toString();
							jiancha_code = listData.get(arg2).get("jiancha_code").toString();
							jiancha_zhuangtai = listData.get(arg2).get("jiancha_zhuangtai").toString();
							baogao_name = listData.get(arg2).get("jiancha_mingcheng").toString();
							hospital_name = listData.get(arg2).get("hospital_name").toString();
							jiancha_keshi_name = listData.get(arg2).get("jiancha_keshi_name").toString();
							goJianyanReport();
						}
					});
				}
	...
	public void goJianyanReport() {
		...
		fragmentTransaction.replace(R.id.contentView, JianyanReport, "left");
