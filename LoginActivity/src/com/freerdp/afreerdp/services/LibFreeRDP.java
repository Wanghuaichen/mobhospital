...
public class LibFreeRDP {
	...
	private static native void freerdp_set_performance_flags(int inst,
			boolean remotefx, boolean disableWallpaper,
			boolean disableFullWindowDrag, boolean disableMenuAnimations,
			boolean disableTheming, boolean enableFontSmoothing,
			boolean enableDesktopComposition);

	private static native void freerdp_set_advanced_settings(int inst,
			String remoteProgram, String workDir);

	private static native void freerdp_set_data_directory(int inst,
			String directory);

	private static native boolean freerdp_update_graphics(int inst,
			Bitmap bitmap, int x, int y, int width, int height);

	private static native void freerdp_send_cursor_event(int inst, int x,
			int y, int flags);

	private static native void freerdp_send_key_event(int inst, int keycode,
			boolean down);

	private static native void freerdp_send_unicodekey_event(int inst,
			int keycode);
	...
	public static interface UIEventListener {
		...
		boolean OnVerifiyCertificate(String subject, String issuer,
				String fingerprint);
	...
	public static boolean setConnectionInfo(int inst, BookmarkBase bookmark) {
		...
		/*
		 * if(bookmark.getType() == BookmarkBase.TYPE_MANUAL) { port =
		 * bookmark.getPort(); hostname = bookmark.getHostname(); } else {
		 * assert false; return false; }
		 */

		freerdp_set_connection_info(inst, hostname, bookmark.getUsername(),
				bookmark.getPassword(), bookmark.getDomain(),
				screenSettings.getWidth(), screenSettings.getHeight(),
				screenSettings.getColors(), port, bookmark
						.getAdvancedSettings().getConsoleMode(), bookmark
						.getAdvancedSettings().getSecurity(), certName);

		BookmarkBase.PerformanceFlags flags = bookmark
				.getActivePerformanceFlags();
		freerdp_set_performance_flags(inst, flags.getRemoteFX(),
				!flags.getWallpaper(), !flags.getFullWindowDrag(),
				!flags.getMenuAnimations(), !flags.getTheming(),
				flags.getFontSmoothing(), flags.getDesktopComposition());

		BookmarkBase.AdvancedSettings advancedSettings = bookmark
				.getAdvancedSettings();
		freerdp_set_advanced_settings(inst,
				advancedSettings.getRemoteProgram(),
				advancedSettings.getWorkDir());
	...
	public static boolean updateGraphics(int inst, Bitmap bitmap, int x, int y,
	...
	private static void OnSettingsChanged(int inst, int width, int height,
			int bpp) {
		SessionState s = GlobalInfoApplication.getSession(inst);
		if (s == null)
			return;
	...
	private static boolean OnAuthenticate(int inst, StringBuilder username,
	...
	private static boolean OnVerifyCertificate(int inst, String subject,
		...
		UIEventListener uiEventListener = s.getUIEventListener();
		if (uiEventListener != null)
			return uiEventListener.OnVerifiyCertificate(subject, issuer,
					fingerprint);
		return false;
	}

	private static void OnGraphicsUpdate(int inst, int x, int y, int width,
			int height) {
		SessionState s = GlobalInfoApplication.getSession(inst);
		if (s == null)
			return;
